...
const Home = () => {
    ...
    const [contactNoHelperText, setContactNoHelperText] = useState('');
    const [selectedSortMathod, setSelectedSortMathod] = useState('');

    const selectSortChange = (event) => {
        setSelectedSortMathod(event.target.value);
    };
    ...
    const handleContactNoChange = (event) => {
        ...
        // Allow only numbers and prevent any non-numeric input
        if (/^[0-9]*$/.test(value)) {
            setContactNo(value);

            const contactNoRegex = /^[0-9]{10}$/; // 10 digits check
            if (value === '') {
                setContactNoError(false);
                setContactNoHelperText('');
            } else if (!contactNoRegex.test(value)) {
                setContactNoError(true);
                setContactNoHelperText('Please enter a valid 10-digit mobile number');
            } else {
                setContactNoError(false);
                setContactNoHelperText('');
            }
